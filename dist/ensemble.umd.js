...
`,ls=m.div`
    ...
    margin-right:.5rem;
    color: ${e=>e.fill};
`,cs=({fill:e,leftText:n="",rightText:t="",Icon:r})=>F(ss,{children:[d(ls,{fill:e,children:d(r,{fill:e})}),d(xr,{fill:e,leftText:n,rightText:t})]}),ot=m.css`
...
`,Ds=m.div`
    font-size: 1.4rem;
`,Ns=({id:e,name:n,inputBackground:t=null,background:r=!1,inputColour:a=null,inputBorder:i=null,prepend:o=null,append:s=null,label:c=null,type:f="text",caption:u=null,required:p=!1,placeholder:h=!1,defaultValue:y=null,status:P=null,...O})=>{let A={inputBackground:"white",inputColour:"black",inputBorder:de.main.border};return r&&(A={inputBackground:de.form.input.background,inputColour:de.form.input.color,inputBorder:de.main.border}),t!==null&&(A={...A,inputBackground:t}),a!==null&&(A={...A,inputColour:a}),i!==null&&(A={...A,inputBorder:i}),F(Cn,{htmlFor:n,block:!0,children:[c!==null?d($n,{required:p,children:c}):null,F(Ls,{...O,...A,children:[o?d(zs,{children:o}):null,d(js,{type:f,name:n,required:p,placeholder:h,defaultValue:y,id:e,...A,...O}),s?d(Ds,{children:s}):null]}),d(Ms,{...A,children:u})]})},Bs=m.div`
    display: flex;
    flex-direction: column;
    width: 100%;
`,Ws=m.select`
...
`,Cr=m($r)``,Qs=m.div`
    margin: 0 0.5rem;
    color: ${({theme:e})=>e.main.color};
`;function el({label:e,name:n,hourMin:t=0,hourMax:r=23,hourValue:a,minuteMin:i=0,minuteMax:o=59,minuteValue:s}){const[c,f]=G.useState(a),[u,p]=G.useState(s);G.useEffect(()=>{f(a),p(s)},[a,s]);function h(){const P=[];for(let O=t;O<=r;O++)P.push(O<10?`0${O}`:O);return P}function y(){const P=[];for(let O=i;O<=o;O++)P.push(O<10?`0${O}`:O);return P}return F(Ks,{children:[d($n,{children:e}),F(Js,{children:[d(Cr,{value:c,name:`${n}[hour]`,inline:!0,onChange:P=>f(P.target.value),children:h().map(P=>d("option",{value:P,children:P},`${n}-${P}`))},`${n}-hour`),d(Qs,{children:":"}),d(Cr,{value:u,name:`${n}[minute]`,inline:!0,onChange:P=>p(P.target.value),children:y().map(P=>d("option",{value:P,children:P},`${n}-${P}`))},`${n}-minute`)]})]})}const nl=m.textarea`
    width: 100%;
    height: 10rem;
    padding: 0.5rem;
    margin: 0 0 1rem;
...
`;function Qf({data:e,rowPadding:n=null,getIsChecked:t=()=>{}}){const{headings:r}=e,{rows:a}=e,[i,o]=G.useState(!1),[s,c]=G.useState([]);G.useEffect(()=>{t(s)},[s]);const f=p=>{o(!i),c(e.rows.map(h=>h.id)),i&&c([])};function u(p){const{checked:h,value:y}=p.target;if(console.log("checked",h,"value",y,s),h){if(s.includes(y))return;c([...s,parseInt(y)])}else o(!1),c(s.filter(P=>P!==parseInt(y)))}return d(Vf,{children:F(Kf,{children:[F(Zt,{border:!0,rowPadding:n,className:"tableHeader",children:[d(Xt,{children:d(st,{margin:"0px",onChange:f,checked:i,value:"0"})}),r.map(p=>d(Xt,{columns:r.length,children:d(Me,{fontWeight:"bold",children:p})},p))]}),a.map((p,h)=>F(Zt,{rowPadding:n,children:[d(Un,{className:"rowCheckbox",children:d(st,{margin:"0px",checked:s.includes(p.id),value:p.id,onChange:y=>u(y)})}),Object.keys(p).map((y,P)=>y==="id"?null:d(Un,{border:!0,justify:y==="actions"?"flex-end":"flex-start",className:y==="actions"?"actions":null,columns:r.length,children:y!=="actions"?d(Jf,{children:p[y]}):p[y]},`${y}-${h}-${P}`))]},`${p.name}-${h}`))]})})}const eu=m.div`
    color: ${e=>e.theme.main.color};
